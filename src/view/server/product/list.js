...
class ProductList extends Component {
  ...
  render() {
    const clickUpdate = (id) =>{
      this.props.history.push('/admin/product/' + id + '/update')
    }
    ...
    if (!this.state.loading && Cookie.get('token')) {
      return (
        <div className="DataList product-list">
          ...
          <Link to="/admin/product/create"><button className="btn btn-primary"> Create A New Product</button></Link>
            ...
            <div className="DataList-filter" onSubmit={this.filterSubmmitHandle}>
              <form className="form-inline w-100">
                <input className="form-control"
                  ...
                  type="text" placeholder="Tìm kiếm Theo Name"
                />

                <input className="form-control"
                  onChange={e=>this.setState({filter:{...this.state.filter,id:e.target.value}})}
                  type="text" placeholder="Tìm kiếm Theo ID"
                />
